...
/*!
 * \file add_rms_norm_dynamic_quant_ag_normal_kernel.h
 * \brief
 */

#ifndef ADD_RMS_NORM_DYNAMIC_QUANT_AG_NORMAL_KERNEL_H_
#define ADD_RMS_NORM_DYNAMIC_QUANT_AG_NORMAL_KERNEL_H_

#include "add_rms_norm_dynamic_quant_ag_base.h"

template <typename T, typename T_Y, int TILING_KEY, int BUFFER_NUM = 1>
class KernelAddRmsNormDynamicQuantAGNormal : public KernelAddRmsNormDynamicQuantAGBase<T, T_Y, TILING_KEY, BUFFER_NUM> {
public:
    __aicore__ inline KernelAddRmsNormDynamicQuantAGNormal(TPipe* pipe)
    {
        Ppipe = pipe;
    }
        ...
        const AddRmsNormDynamicQuantAGTilingData* tiling)
    {
        ...
        if(this->blockIdx_ < this->numCore) {
            ...
            uint32_t offsetScale = this->rowLen * this->rowTotalNum * sizeof(int8_t);
